...
@interface ViewController () <UIAlertViewDelegate, GJSPayProtocol, UPAPayPluginDelegate> {
    //
    GJSPay *pay;
}
...
- (IBAction)testApplePay:(id)sender {
    [self.navigationController pushViewController:[[UPViewController alloc] init] animated:YES];
    return;
    PaySDK = ApplePayCUPType;
    //GJSPay *pay = [[GJSPay alloc] init];
    pay = [[GJSPay alloc] init];
    pay.delegate = self;
    OrderForUnionPay *order = [[OrderForUnionPay alloc] initWithDictionary:@{@"TradeNO": @"201602181039576614948", @"ProductName": @"测试商品名称", @"ProductDescription": @"测试商品描述", @"Amount": @"0.01", @"NotifyURL": @"www.xxx.com"}];
    [pay payByApplePayCUPOrder:(id)order viewController:self];
}
...
- (void)UPAPayPluginResult:(UPPayResult *)payResult {
    if(payResult.paymentResultStatus == UPPaymentResultStatusSuccess) {
        ...
        [GJSUtils showMessage:successInfo];
