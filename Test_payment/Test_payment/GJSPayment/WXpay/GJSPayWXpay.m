...
@interface GJSPayWXpay () <WechatPayManagerDelegate> {
...
+(BOOL) handleOpenURL:(NSURL *) url delegate:(id<GJSPayProtocol>) delegate {
    GJSPayWXpay *pureSelf = [[GJSPayWXpay alloc] initPure];
    pureSelf.delegate = delegate;
    [WechatPayManager sharedManager].delegate = (id<WechatPayManagerDelegate>)pureSelf;
...
#pragma mark --
#pragma mark WechatPayManagerDelegate
...
- (void)managerDidRecvPayResponse:(PayResp *)response {
    ...
    if ([response isKindOfClass:[PayResp class]]){
        switch(response.errCode){
            ...
            {
                strMsg = @"支付结果：成功！";
                NSNotification *notification = [NSNotification notificationWithName:ORDER_PAY_NOTIFICATION object:@"success" userInfo:[NSDictionary dictionaryWithObjectsAndKeys:WXpayType,@"payType", strMsg,@"message", response,@"response", nil]];
                [[NSNotificationCenter defaultCenter] postNotification:notification];
                //交易成功
                if (self.delegate && [self.delegate respondsToSelector:@selector(paymentDidSuccess:responseInfo:payType:)]) {
                    [self.delegate paymentDidSuccess:nil responseInfo:[NSDictionary dictionaryWithObjectsAndKeys:WXpayType,@"payType", strMsg,@"message", response,@"response", nil] payType:WXpayType];
                }
            }
                break;
            ...
            {
                strMsg = @"支付结果：失败！";
                NSNotification *notification = [NSNotification notificationWithName:ORDER_PAY_NOTIFICATION object:@"fail" userInfo:[NSDictionary dictionaryWithObjectsAndKeys:WXpayType,@"payType", strMsg,@"message", response,@"response", nil]];
                [[NSNotificationCenter defaultCenter] postNotification:notification];
                //交易失败
                if (self.delegate && [self.delegate respondsToSelector:@selector(paymentDidFailed:responseInfo:payType:)]) {
                    [self.delegate paymentDidFailed:nil responseInfo:[NSDictionary dictionaryWithObjectsAndKeys:WXpayType,@"payType", strMsg,@"message", response,@"response", nil] payType:WXpayType];
                }
            }
                break;
            ...
            {
                strMsg = @"用户已经退出支付！";
                NSNotification *notification = [NSNotification notificationWithName:ORDER_PAY_NOTIFICATION object:@"cancel" userInfo:[NSDictionary dictionaryWithObjectsAndKeys:WXpayType,@"payType", strMsg,@"message", response,@"response", nil]];
                [[NSNotificationCenter defaultCenter] postNotification:notification];
                //交易取消
                if (self.delegate && [self.delegate respondsToSelector:@selector(paymentDidCanceled:responseInfo:payType:)]) {
                    [self.delegate paymentDidCanceled:nil responseInfo:[NSDictionary dictionaryWithObjectsAndKeys:WXpayType,@"payType", strMsg,@"message", response,@"response", nil] payType:WXpayType];
                }
            }
                break;
